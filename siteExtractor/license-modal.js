...
if (!modal) {
  addElement(document.body, 'div', 'cssscan-license-modal',
    `
  ...
  window.onclick = function(event) {
    ...
    if (event.target === licenseBtn && licenseInput.value.length > 20) {
      event.target.innerHTML = `<div class="cssscan-vertical-align"><img src="${chrome.extension.getURL('spinner.svg')}" width="15" class="cssscan-margin-right cssscan-spinner">Checking ...</div>`
      event.target.disabled = true
      chrome.runtime.sendMessage({ action: 'validate_license', key: licenseInput.value.trim() }, function(response) {})
    }
...
},{"./license-modal-styles":1,"axios":3}],3:[function(require,module,exports){
